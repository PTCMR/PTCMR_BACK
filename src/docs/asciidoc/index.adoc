= PTCMR API
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 1
:sectlinks:


== 상품 생성

=== 요청

include::{snippets}/product-create/http-request.adoc[]

include::{snippets}/product-create/request-fields.adoc[]

=== 응답

include::{snippets}/product-create/http-response.adoc[]

== 상품 삭제

=== 요청

include::{snippets}/product-delete/http-request.adoc[]

include::{snippets}/product-delete/path-parameters.adoc[]

== 상품 수정

=== 요청

include::{snippets}/product-update/http-request.adoc[]

include::{snippets}/product-update/request-fields.adoc[]

include::{snippets}/product-update/path-parameters.adoc[]

=== 응답

include::{snippets}/product-update/http-response.adoc[]

== 상품 조회

=== 요청

include::{snippets}/product-detail/http-request.adoc[]

include::{snippets}/product-detail/path-parameters.adoc[]

=== 응답

include::{snippets}/product-detail/http-response.adoc[]

== 상품 페이징

include::{snippets}/product-pagination/http-request.adoc[]

include::{snippets}/product-pagination/request-fields.adoc[]

=== 응답

include::{snippets}/product-pagination/http-response.adoc[]

== 팀 생성

=== 요청

include::{snippets}/team-create/http-request.adoc[]

include::{snippets}/team-create/request-fields.adoc[]

=== 응답

include::{snippets}/team-create/http-response.adoc[]

include::{snippets}/team-create/request-fields.adoc[]

=== 응답

include::{snippets}/team-create/http-response.adoc[]

== 팀 수정

=== 요청

include::{snippets}/team-update/http-request.adoc[]

include::{snippets}/team-update/request-fields.adoc[]

=== 응답

include::{snippets}/team-update/http-response.adoc[]

== 팀 삭제

=== 요청

include::{snippets}/team-delete/http-request.adoc[]

include::{snippets}/team-delete/path-parameters.adoc[]

=== 응답

include::{snippets}/team-delete/http-response.adoc[]

== 팀 초대

=== 요청

include::{snippets}/team-invite/http-request.adoc[]

include::{snippets}/team-invite/request-fields.adoc

=== 응답

include::{snippets}/team-invite/http-response.adoc[]

== 카테고리 생성

=== 요청

include::{snippets}/category-create/http-request.adoc[]

include::{snippets}/category-create/request-fields.adoc[]

=== 응답

include::{snippets}/category-create/http-response.adoc[]

== 카테고리 수정

=== 요청

include::{snippets}/category-update/http-request.adoc[]

include::{snippets}/category-update/request-fields.adoc[]

include::{snippets}/category-update/path-parameters.adoc[]

=== 응답

include::{snippets}/category-update/http-response.adoc[]

== 카테고리 삭제

=== 요청

include::{snippets}/category-delete/http-request.adoc[]

include::{snippets}/category-delete/request-fields.adoc[]

=== 응답

include::{snippets}/category-delete/http-response.adoc[]

== 카테고리 페이징

=== 요청

include::{snippets}/category-pagination/http-request.adoc[]

include::{snippets}/category-pagination/request-fields.adoc[]

=== 응답

include::{snippets}/category-pagination/http-response.adoc[]
